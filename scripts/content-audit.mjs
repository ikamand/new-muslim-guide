...
/*
  Progress keys survive renames, or fail loudly.

  Ticks and bookmarks live on devices as `kind:id` strings; renaming a content
  id orphans every one of them, silently, over the air. The guard: a committed
  snapshot of every key progress can attach to, compared against the catalogue.
  A key that disappears must have an entry in PROGRESS_KEY_MIGRATIONS
  (src/content/progress-keys.ts) or this exits non-zero — a fault, not a gap,
  so it fails without --strict. New keys rewrite the snapshot in place; commit
  the file with the content that changed it.
*/
...
const progressKeys = new Set(
  CATALOG.filter((entry) => PROGRESS_KINDS.has(entry.kind)).map(label),
);
const snapshotPath = join(root, 'docs/progress-keys.txt');
const committedKeys = existsSync(snapshotPath)
...
const vanishedKeys = committedKeys.filter(
  (key) => !progressKeys.has(key) && !(key in PROGRESS_KEY_MIGRATIONS),
);
const deadMigrations = Object.entries(PROGRESS_KEY_MIGRATIONS)
  .filter(([, to]) => !progressKeys.has(to))
  .map(([from, to]) => `${from} → ${to}`);
const staleMigrations = Object.keys(PROGRESS_KEY_MIGRATIONS).filter((from) =>
  progressKeys.has(from),
);

if (vanishedKeys.length) {
  failures.push(
    `${vanishedKeys.length} progress key(s) vanished from the catalogue with no entry in ` +
      `PROGRESS_KEY_MIGRATIONS — every device holding them loses that progress: ` +
      vanishedKeys.join(', '),
  );
}
if (deadMigrations.length) {
  failures.push(
    `${deadMigrations.length} PROGRESS_KEY_MIGRATIONS entr(ies) pointing at a key that does not exist: ` +
      deadMigrations.join(', '),
  );
}
if (staleMigrations.length) {
  failures.push(
    `${staleMigrations.length} PROGRESS_KEY_MIGRATIONS entr(ies) whose OLD key still exists — ` +
      `a migration away from a live key rewrites progress that was never orphaned: ` +
      staleMigrations.join(', '),
  );
}

const wantSnapshot =
  '# Every key progress can attach to (ticks, bookmarks, finish history).\n' +
  '# Regenerated by `npm run content:audit`. A key removed from this list\n' +
  '# without an entry in src/content/progress-keys.ts fails the audit.\n' +
  [...progressKeys].sort().join('\n') +
  '\n';
if (
  !vanishedKeys.length &&
  (!existsSync(snapshotPath) || readFileSync(snapshotPath, 'utf8') !== wantSnapshot)
) {
  writeFileSync(snapshotPath, wantSnapshot);
  // `out` was flushed above, so this reports directly.
  console.log(`Progress-key snapshot rewritten: docs/progress-keys.txt (${progressKeys.size} keys).`);
}
