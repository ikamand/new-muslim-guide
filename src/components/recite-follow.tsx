 ...
 *
 * The five rules bind as before; the one visible here is that nothing in
 * this bar can express a score, a count of mistakes, or a "wrong".
 */
...
export function ReciteControls({ follow }: { follow: ReciteFollowController }) {
  const theme = useTheme();
  const { t } = useLocale();
  if (!follow.open) return null;

  const busy = follow.state === 'downloading' || follow.state === 'starting';
